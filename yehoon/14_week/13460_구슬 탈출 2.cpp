...
// 빨간 구슬이 움직이는 방향(상하좌우) 삽입하는 함수
void q_push_dir(q_input arg_q_input) {
    for (int i = 0; i < 4; i++) {
        ...
        // 빨간, 파란 구슬의 위치가 동일하고 기울이는 방향도 동일할 시 큐에 삽입하지 않는다.
...
int main(void) {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            if (arr[i][j] == 'R') {
                init_arg_push_dir.cur_red = {i, j};
            } 
            else if (arr[i][j] == 'B') {
                init_arg_push_dir.cur_blue = {i, j};
            } 
            else if (arr[i][j] == 'O') {
                hole = {i, j};
            }
